## Loads and indexes creature definition resources from disk.
##
## CreatureRegistry scans a directory for [code].tres[/code] resources, loads
## any that are [CreatureDef], and indexes them by their declared [code]id[/code].
## It is the read-only catalog the rest of the game queries when it needs to
## resolve a creature id to its definition or list available creatures.
##
...
##
## Creatures are returned sorted by id for a stable, deterministic order.
##
...
## Scan [param dir] for [CreatureDef] resources to index.
##
## The directory is optional; a missing or unreadable directory contributes
## nothing rather than raising. Resources that are not [CreatureDef], or that
## carry an empty id, are skipped.
func _init(dir := "res://resources/creatures") -> void:
	_scan(dir)


## Return the [CreatureDef] registered under [param id], or [code]null[/code].
func creature(id: StringName) -> CreatureDef:
	return _creatures.get(id, null)


## Return every registered [CreatureDef] sorted by id for stable order.
func creatures() -> Array[CreatureDef]:
	var ids := creature_ids()
...
## Return the ids of every registered creature sorted alphabetically.
func creature_ids() -> Array[StringName]:
...
func _scan(dir_path: String) -> void:
	for res in _load_resources(dir_path):
		var def := res as CreatureDef
		if def == null or def.id == &"":
			continue
		_creatures[def.id] = def
