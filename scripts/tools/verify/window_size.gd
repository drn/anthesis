# SceneTree harness, NOT a GUT test. Boots the real world windowed, waits for
# terrain, snaps the framebuffer, and reports the window/viewport size so the
# configured 1920x1080 default can be verified live.
extends SceneTree
...
func _process(_delta: float) -> bool:
	...
	if _frame == 180:
		...
		var out_path := out_dir + "/window-size.png"
		var err := img.save_png(out_path)
		print("SNAP %s err=%d" % [out_path, err])
